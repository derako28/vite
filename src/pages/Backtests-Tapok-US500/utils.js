...
export const getChartConfigForBacktest = (data = [], getData, propertyRR = propertyForResult, width = 300, height = 300) => {
    return {
...
export const getDataLine3 = (dataBefore, dataAfter) => {
    ...
    return Object.keys(prepareData).map((key) => {
        return {date: new Date(moment(key, "MM-DD-YYYY").format()), profit: prepareData[key]}
    })
}
...
export const assignResult = (data, property = '1r') => {
    return data.map((item) => ({...item, result: item[property] === 'Yes' ? 'win' : 'lose'}))
}
